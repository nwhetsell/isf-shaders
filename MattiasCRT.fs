...
void main()
{
    ...
    if (uv.x < 0. || uv.x > 1. ||
        ...
        float x = sin( 0.30 * TIME + uv.y * 21.) *
                  ...
                  0.0017;
        vec2 northeast = vec2(uv.x + 0.001, uv.y + 0.001);
        ...
        color *= 2.8;
        float scans = clamp(0.35 + 0.35 * sin(3.5 * TIME + uv.y * RENDERSIZE.y * 1.5), 0., 1.);
        color *= vec3(0.4 + 0.7 * pow(scans, 1.7));
        color *= 1. + 0.01 * sin(110. * TIME);
        color *= 1. - 0.65 * vec3(clamp((mod(gl_FragCoord.x, 2.) - 1.) * 2., 0., 1.));
